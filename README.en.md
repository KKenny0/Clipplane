...
Clipplane's public release is still a GitHub dev preview; the current source is now the `0.6.1` Chrome Web Store candidate:
...
- The current `0.6.1` source and Chrome Web Store draft use canonical extension ID `emacefnmbogjdcblglmipolnickjnmbl`.
...
Chrome Native Messaging requires the local host to list the exact extension origins allowed to access it. Wildcards are not allowed. Clipplane's `manifest.key` is the public identity key from Chrome Web Store, keeping a locally loaded `0.6.1` candidate aligned with the Store Item ID. It is not a signing private key, and the repo does not store `.pem` files or other private keys. Store packaging removes this field from the staging copy before upload.
...
5. Current source and `0.6.1` candidates should show `emacefnmbogjdcblglmipolnickjnmbl`; the published `v0.5.0` dev-preview package still shows legacy ID `mhgcfphfcgbgabhbegdonadkedfaddhc`.
