...
Clipplane is a local-first browser clipper. It saves the current page or selected text to your local `inbox.org` first, then optionally syncs the saved capture to Notion or flomo.
...
- The local host and setup scripts still come from the source repo or the Release Source code package.
...
Why the extension ID is still required: Chrome Native Messaging requires the local host to list the exact extension origins allowed to access it. Wildcards are not allowed. Before store distribution, manually loaded extension IDs are generated by the browser, so the setup script needs that ID to write the local manifest.
...
- `Source code`: contains the local host and setup scripts.
- `clipplane-extension-vX.zip`: the browser extension package.

Unzip both into stable folders, then run this in the `Source code` folder. If you are running directly from the Git repo, run the same commands from the repo root:
...
`smoke` writes a temporary `inbox.org` under the system temp directory so you can confirm the local clip core works.
