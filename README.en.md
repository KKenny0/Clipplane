...
Clipplane's public release is still a GitHub dev preview; the current source is now the `0.7.2` Chrome Web Store candidate:
...
- The current `0.7.2` source and Chrome Web Store draft use canonical extension ID `emacefnmbogjdcblglmipolnickjnmbl`.
...
5. Current source and `0.7.2` candidates should show `emacefnmbogjdcblglmipolnickjnmbl`; the published `v0.5.0` dev-preview package still shows legacy ID `mhgcfphfcgbgabhbegdonadkedfaddhc`.
